...
{
    public class HostileProj_0 : ModProjectile // 
    {
        public override string Texture => "MyMod/TuPian/Trail1";//这里教一下直接引用本模组内的图片，这样使用同图片的弹幕就不需要准备一堆图片了
        ...
        {
            Player player = Main.player[Projectile.owner];//用owner来找玩家
            Projectile.localAI[1]++;//在前1.5s内半追踪玩家
            if (Projectile.localAI[1] < 90f)
            {
                ...
                else
                {
                    ...
                    float num7 = num6.AngleLerp(targetAngle, 0.01f);//角度渐变
        ...
        public override bool PreDraw(ref Color lightColor)//predraw返回false即可禁用原版绘制
        {
            ...
            Color MyColor = Color.Red; MyColor.A = 0;//让A=0是为了能直接叠加颜色
