...
using MyMod.Content.ModProj.BossProjectile;
using System;
using System.IO;
using Terraria;
...
namespace MyMod.Content.NPCs // 命名空间咱们写文件夹路径
{
                ...
                case 4://本状态为过场动作，锁定血量并且旋转，抛出电火花
                    {
                        ...
                        {
                            float targetRotation = NPC.AngleTo(player.Center);
                            NPC.rotation = NPC.rotation.AngleLerp(targetRotation, 0.1f);//朝向扶正，面对玩家，准备战斗
                        }
                        else
                        {
                ...
                case 5://本状态为喷火
                    {
                        ...
                        NPC.rotation = NPC.rotation.AngleLerp(NPC.AngleTo(player.Center), 0.04f);//角度渐变
                ...
                case 6://本状态进行5~7（随难度增加）次短程快速冲刺，最后进行一次长程预判冲刺
                    {
                        ...
                        else //这时候说明前几次冲刺结束了，那么我们进行一次预判大冲刺
                        {
                            ...
                            if (NPC.ai[0] < 80)//在计时器从39到达80这段时间内先减速，并且瞄准预判玩家。
                            {
                                ...
                                NPC.rotation = NPC.rotation.AngleLerp(NPC.AngleTo(offset), 0.07f);//角度渐变
                ...
                case 8://本状态BOSS将进入3阶段(专家以上)，进行咆哮和震屏
                    {
                        ...
                        NPC.rotation = NPC.rotation.AngleLerp(NPC.AngleTo(player.Center), 0.05f);
